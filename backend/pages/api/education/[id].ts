...
async function handleGetById(req: NextApiRequest, res: NextApiResponse, id: string) {
  try {
    ...
    const educationContent = await Education.findById(id).populate("author", "displayName email").lean();
...
async function handleUpdate(req: NextApiRequest, res: NextApiResponse, id: string) {
  ...
  try {
    ...
    const educationContent = await Education.findByIdAndUpdate(id, updateData, { new: true }).populate("author", "displayName email");
