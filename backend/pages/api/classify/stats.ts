...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const classificationsByType = await Classification.aggregate([{ $match: { userId } }, { $group: { _id: "$classificationResult", count: { $sum: 1 } } }, { $sort: { count: -1 } }]);

    // Get classifications by method
    const classificationsByMethod = await Classification.aggregate([{ $match: { userId } }, { $group: { _id: "$method", count: { $sum: 1 } } }]);
    ...
    const avgConfidenceResult = await Classification.aggregate([{ $match: { userId } }, { $group: { _id: null, avgConfidence: { $avg: "$confidence" } } }]);
